...
class MSOfficeParserController(http.Controller):
    ...
    @http.route('/web/preview/converter/msoffice', auth="public", type='http')
    ...
    def _get_response(self, url, export_filename):
        ...
        if not bool(urlparse.urlparse(url).netloc):
            ...
            try:
                ...
                print self._make_error_response(exception.response.status_code, exception.response.reason or _("Unknown Error"))
                print "----------------------------------------------------------------------------------------------"
                return self._make_error_response(exception.response.status_code, exception.response.reason or _("Unknown Error"))
        try:
            par = urlparse.parse_qs(urlparse.urlparse(url).query)
            filenamefromurl = "".join(par['filename'])
            resp = pdfconv.converter.convert_binary2pdf(data,None,filenamefromurl.encode('utf8'), format='binary')
            response = self._make_pdf_response(resp, export_filename or uuid.uuid4())
            pdf_cache[url] = response
            print "----hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh"
            return response
        except KeyError:
